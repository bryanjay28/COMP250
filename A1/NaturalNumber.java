...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		int carry = 0;
		int temp, i;
		int numberSize = firstClone.coefficients.size();
		
		for (i=0; i < numberSize; i++){
			temp = firstClone.coefficients.get(i) + secondClone.coefficients.get(i) + carry;
			carry = temp/base; 
			sum.coefficients.addLast(temp%base) ;
		}
		if (carry != 0)
	...
	public NaturalNumber slowTimes( NaturalNumber  multiplier) throws Exception{
		...
		for (NaturalNumber counter = new NaturalNumber(0, this.base);
				counter.compareTo(multiplier) < 0;  counter = counter.plus(one) ){
				prod = prod.plus(this);
		}
		return prod;
	}
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		for(i = 0; i < multiplier.coefficients.size(); i++){
			int singleDigit = multiplier.coefficients.get(i);
			product = product.plus(timesSingleDigit(singleDigit, multiplicandClone).timesBaseToThePower(i));
	...
	public NaturalNumber timesSingleDigit (int singleDigit, NaturalNumber multiplicand){
		
		NaturalNumber product = new NaturalNumber(multiplicand.base);
		int i, temp, carry = 0;
		
		int base = multiplicand.base;
		int numberSize = multiplicand.coefficients.size();
		
		for(i = 0; i < numberSize; i++){
			temp = (multiplicand.coefficients.get(i) * singleDigit) + carry;
			
			carry = temp/base;
			product.coefficients.addLast(temp%base);		
		}
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		int i;
		int numbSize = firstClone.coefficients.size();
		for(i = 0; i < numbSize; i++){
			
			if(firstClone.coefficients.get(i) >= secondClone.coefficients.get(i)){
				difference.coefficients.add((firstClone.coefficients.get(i) - secondClone.coefficients.get(i)));				
				
			} else {
				int temp = firstClone.coefficients.get(i+1);
				temp -= 1;
				firstClone.coefficients.set(i + 1, temp);
				
				difference.coefficients.add(firstClone.coefficients.get(i) + base - secondClone.coefficients.get(i));
			}
		}
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
		...
		NaturalNumber  tempValue;
		int zeroCounter = 0;
		
		NaturalNumber divisorClone = divisor.clone();

		while(remainder.compareTo(divisorClone) > 0){
			...
			zeroCounter -= 1;
			tempDivisor.coefficients.remove();
			
			
			if(remainder.coefficients.get(remainder.coefficients.size() - 1) > tempDivisor.coefficients.get(tempDivisor.coefficients.size() - 1)){
				while(remainder.coefficients.size() != tempDivisor.coefficients.size()){
					 tempDivisor.coefficients.addFirst(0);
					 zeroCounter++;
				}
			} else {
			...
			int quotientDigit = 0;
			while(timesSingleDigit(quotientDigit + 1, tempDivisor).compareTo(remainder) < 0){
				quotientDigit++;				
			}
			
			remainder = remainder.minus(timesSingleDigit(quotientDigit, tempDivisor));
			tempValue = new NaturalNumber(quotientDigit, this.base);

			
			while(zeroCounter > 0){
				tempValue.coefficients.addFirst(0);
				zeroCounter--;
				
			}
			quotient = quotient.plus(tempValue);
