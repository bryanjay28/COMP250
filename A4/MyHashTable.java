...
class MyHashTable<K,V> {
	...
	public  V  put(K key, V value) {
		...
		//  ADD YOUR CODE BELOW HERE
		int keyPosition = hashFunction(key);
		HashLinkedList<K,V> tempList = buckets.get(keyPosition);
		HashNode<K,V> node;
		V removeValue;

			if (this.containsKey(key)){
				
				node = tempList.remove(key);
				tempList.add(key, value);
				buckets.set(keyPosition, tempList);
		...
		double loadFactor = (double) this.entryCount / this.numBuckets;
	...
	public V get(K key) {
		...
		int keyPosition = hashFunction(key);
		if(this.containsKey(key)) {
			return buckets.get(keyPosition).getListNode(key).getValue();
		} else {
			return null;
		}
	...
	public V remove(K key) {
		...
		//  ADD YOUR CODE BELOW HERE
		int keyNumber = hashFunction(key);
		
		HashLinkedList<K,V> tempList;
		HashNode<K,V> removedNode;
		
		if (this.containsKey(key)) {
			tempList = buckets.get(keyNumber);
			removedNode = tempList.getListNode(key);
	...
	{
		//   ADD YOUR CODE BELOW HERE
		int newSize = this.getNumBuckets() * 2;
		
		ArrayList<HashLinkedList<K,V>> newList = new ArrayList<HashLinkedList<K, V>>(newSize);
		...
		while(iterator.hasNext()) {
			...
			this.put(tempNode.getKey(), tempNode.getValue());
		}
	...
	public class HashIterator implements  Iterator<HashNode<K,V> > {
		...
		public  HashIterator()
		{
			...
			for (int i = 0; i < buckets.size(); i++){
				tempList = buckets.get(i).replicateList(buckets.get(i));
