...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		WordTreeNode largestPrefix = getPrefixNode(word);
		WordTreeNode treeTemp = new WordTreeNode();
		int length = word.length();
		
		for(int i = largestPrefix.depth; i < length; i++) {
			largestPrefix = largestPrefix.createChild(word.charAt(i)); 
			
		}
		
		largestPrefix.setEndOfWord(true);
	...
	{  
		//   ADD YOUR CODE BELOW HERE
		WordTreeNode tempTree = new WordTreeNode();		
		tempTree = getPrefixNode(word);
		
		int length = word.length();
		if(tempTree != null) {
			
			if(tempTree.depth == length && tempTree.isEndOfWord()) {
				return true;
			} 			
		} 
		return false;
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
		...
		WordTreeNode tempTree = getPrefixNode(prefix);
		
		if(tempTree.depth == prefix.length()) {
			totalWords = preOrderTreeTraverse(tempTree, totalWords);
		}
		return totalWords; 
	...
	public class WordTreeNode
	{
		...
		public String toString()
		{
			...
			if(depth != 0 && this.parent != null) {
				...
				word = this.charInParent;
				return this.parent.toString() + word;
	 ...
	 * Added Helper Method
	 */
	public ArrayList<String> preOrderTreeTraverse(WordTreeNode treeNode, ArrayList<String> wordMatches) {
		
		if(treeNode.endOfWord) {
			wordMatches.add(treeNode.toString());
		}
		
		for(int i = 0; i < WordTreeNode.NUMCHILDREN; i++) {
			if(treeNode.getChild((char) i) == null) {
				continue;
				
			} else {
				
				preOrderTreeTraverse(treeNode.getChild((char) i), wordMatches);
			}
		}
